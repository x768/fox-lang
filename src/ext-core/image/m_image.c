...
static int color_rgb(Value *vret, Value *v, RefNode *node)
{
	...
	for (i = 0; i < argc; i++) {
		int val = fs->Value_int(v[i + 1], NULL);
...
static int color_hsl(Value *vret, Value *v, RefNode *node)
{
	...
	if (fg->stk_top > v + 4) {
		int64_t val = fs->Value_int(v[4], NULL);
		if (val < 0) {
			a = 0;
		} else if (val > 255) {
			a = 255;
...
{
	int c = fs->Value_int(v[1], NULL);
	int m = fs->Value_int(v[2], NULL);
	int y = fs->Value_int(v[3], NULL);
	int k = fs->Value_int(v[4], NULL);
...
{
	int err = FALSE;
	int64_t width = fs->Value_int(v[1], &err);
	int64_t height = fs->Value_int(v[2], &err);
	size_t size_alloc;
	int channels;
	int bands;
	RefImage *image;
...
static int image_index(Value *vret, Value *v, RefNode *node)
{
	RefImage *image = Value_vp(*v);
	int err = FALSE;
	int64_t x = fs->Value_int(v[1], &err);
	int64_t y = fs->Value_int(v[2], &err);
	uint32_t color;
	uint8_t *line;
...
static int image_copy(Value *vret, Value *v, RefNode *node)
{
	...
	int err = FALSE;
	int64_t x = fs->Value_int(v[2], &err);
	int64_t y = fs->Value_int(v[3], &err);
	RefRect *src_rect;
	RefRect src_rect_v;
