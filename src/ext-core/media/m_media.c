...
static int audio_new(Value *vret, Value *v, RefNode *node)
{
	...
	*vret = vp_Value(snd);
	...
	if (fg->stk_top > v + 3) {
		channels = fs->Value_int(v[3], NULL);
...
{
	RefAudio *snd = Value_vp(*v);
	int64_t pos = fs->Value_int(v[1], NULL);
	int64_t ch = fs->Value_int(v[2], NULL);
	int f = FUNC_INT(node);
...
static int audio_index_set(Value *vret, Value *v, RefNode *node)
{
	...
	int64_t pos = fs->Value_int(v[1], NULL);
	int64_t ch = fs->Value_int(v[2], NULL);
	int64_t val = fs->Value_int(v[3], NULL);
