...
static int stream_read(Value *vret, Value *v, RefNode *node)
{
	...
	if (fg->stk_top > fg->stk_base + 1) {
		Value v1 = v[1];
		int64_t size64 = Value_int(v1, NULL);
		int size;
...
static int stream_seek_sub(Value v, int64_t offset)
{
	...
	new_offset = Value_int(fg->stk_top[-1], NULL);
	Value_pop();
...
{
	RefBytesIO *mb;
	int64_t size = Value_int(v[1], NULL);
	int val = 0;
...
static int bytesio_index(Value *vret, Value *v, RefNode *node)
{
	RefBytesIO *mb = Value_vp(*v);
	int size = mb->buf.size;
	int64_t idx = Value_int(v[1], NULL);
...
static int bytesio_index_set(Value *vret, Value *v, RefNode *node)
{
	...
	int size = mb->buf.size;
	int64_t idx = Value_int(v[1], NULL);
	int64_t val = Value_int(v[2], NULL);
...
static int bytesio_fill(Value *vret, Value *v, RefNode *node)
{
	RefBytesIO *mb = Value_vp(*v);
	int size = mb->buf.size;
	int64_t val = Value_int(v[1], NULL);
	int64_t begin = Value_int(v[2], NULL);
	int64_t end = Value_int(v[3], NULL);
...
static int bytesio_write(Value *vret, Value *v, RefNode *node)
{
	...
	if (!StrBuf_add_r(&mb->buf, Value_vp(v[1]))) {
		return FALSE;
	}
...
static int strio_index(Value *vret, Value *v, RefNode *node)
{
	...
	int idx = utf8_position(src_p, src_size, Value_int(v[1], NULL));
		...
		if (port.size > 0) {
			...
			if (port_i >= 0) {
				*vret = int32_Value(port_i);
				...
				throw_errorf(fs->mod_lang, "ValueError", "Unknwon port %Q", port);
				return FALSE;
			}
		} else {
			...
			if (n == PORT_UNKNOWN) {
				throw_errorf(fs->mod_lang, "ValueError", "Unknwon scheme %Q", scheme);
				return FALSE;
			}
			*vret = int32_Value(n);
		}
...
static void define_io_class(RefNode *m)
{
	...
	n = define_identifier_p(m, cls, fs->str_tostr, NODE_FUNC_N, 0);
	define_native_func_a(n, strclass_tostr, 0, 2, NULL, fs->cls_str, fs->cls_locale);
