...
int strclass_tostr(Value *vret, Value *v, RefNode *node)
{
	...
	const Str s = Value_str(*v);
	*vret = printf_Value("%n(%S)", type, s);
	return TRUE;
}
static int sequence_from_iterator(Value *vret, Value *v, int is_str)
{
	...
	for (;;) {
		...
		err = FALSE;
		val = Value_int(fg->stk_top[-1], &err);
		Value_pop();
...
static int sequence_marshal_read(Value *vret, Value *v, RefNode *node)
{
	...
	rs = new_refstr_n((is_str ? fs->cls_str : fs->cls_bytes), size);
	*vret = vp_Value(rs);
	rd_size = size;
...
{
	int32_t num = Value_int(v[1], NULL);
...
static int sequence_match(Value *vret, Value *v, RefNode *node)
{
	...
	if (v1_type == v0_type) {
		...
		if (idx >= 0) {
			r = new_buf(cls_matches, sizeof(RefMatches) + sizeof(int) * 2);
			*vret = vp_Value(r);
			r->m[0].begin = idx;
			r->m[0].end = idx + s1.size;
		...
		pcre_fullinfo(re, NULL, PCRE_INFO_CAPTURECOUNT, &m_count);
		m_size = (m_count + 1) * 3;
		r = new_buf(cls_matches, sizeof(RefMatches) + sizeof(int) * m_size);
		*vret = vp_Value(r);
		n_match = pcre_exec(re, NULL, src.p, src.size, offset, 0, &r->m[0].begin, m_size);
		if (n_match >= 0) {
			r->bin = (v0_type == fs->cls_bytes);
			r->count = n_match;
		} else {
		...
		for (;;) {
			...
			if (callback) {
				...
				*fg->stk_top++ = v2;
				...
				r = new_buf(cls_matches, sizeof(RefMatches) + sizeof(int) * n_match * 2);
				*fg->stk_top++ = vp_Value(r);
				r->count = n_match;
				memcpy(r->m, matches, sizeof(int) * n_match * 2);
				r->bin = (v_type == fs->cls_bytes);
				r->source = Value_cp(*v);
...
static int sequence_splice(Value *vret, Value *v, RefNode *node)
{
	...
	rs = new_refstr_n(v_type, src.size + append.size - len);
	*vret = vp_Value(rs);
...
static int sequence_caseconv(Value *vret, Value *v, RefNode *node)
{
	...
	Str src = Value_str(*v);
	RefNode *v_type = Value_type(*v);
	RefStr *rs = new_refstr_n(v_type, src.size);
	*vret = vp_Value(rs);
...
static int sequence_trim(Value *vret, Value *v, RefNode *node)
{
	...
	if (collapse) {
		...
		if (k < ret.size - 1) {
			int pre = FALSE;
			RefStr *rs = new_refstr_n(v_type, ret.size);
			*vret = vp_Value(rs);
...
static int string_index(Value *vret, Value *v, RefNode *node)
{
	...
	int idx = utf8_position(src->c, src->size, Value_int(v1, NULL));
...
void string_substr_position(int *pbegin, int *pend, Str src, Value *v)
{
	...
	if (fg->stk_top > v + 2) {
		int32_t begin_i = Value_int(v[1], NULL);
		int32_t end_i = Value_int(v[2], NULL);
...
static int bytes_new_seq(Value *vret, Value *v, RefNode *node)
{
	...
	rs = new_refstr_n(fs->cls_bytes, top - v1);
	*vret = vp_Value(rs);
	dst = rs->c;
	rs->c[rs->size] = '\0';
	while (v1 < top) {
...
{
	Str src = Value_str(*v);
	int32_t begin = Value_int(v[1], NULL);
	int32_t end;
	int32_t len;
...
static int regex_tostr(Value *vret, Value *v, RefNode *node)
{
	...
	StrBuf_add_c(&buf, '/');
	add_backslashes_bin(&buf, source.p, source.size);
	StrBuf_add_c(&buf, '/');
...
static int matches_position(Value *vret, Value *v, RefNode *node)
{
	...
	{
		int pos = (end ? r->m[idx].end : r->m[idx].begin);
		if (!r->bin) {
			Str src = Value_str(r->source);
...
static int str_base64encode(Value *vret, Value *v, RefNode *node)
{
	...
	rs = new_refstr_n(fs->cls_str, buf.size * 4 / 3 + 4);
	*vret = vp_Value(rs);
...
static int str_base64decode(Value *vret, Value *v, RefNode *node)
{
	...
	rs = new_refstr_n(fs->cls_bytes, src->size * 3 / 4 + 4);
	*v = vp_Value(rs);
...
static int str_hexencode(Value *vret, Value *v, RefNode *node)
{
	...
	rs = new_refstr_n(fs->cls_str, buf.size * 2);
	*vret = vp_Value(rs);
...
void define_lang_str_class(RefNode *m)
{
	...
	define_native_func_a(n, string_new, 0, 3, NULL, NULL, NULL, fs->cls_bool);
	n = define_identifier(m, cls, "chr", NODE_NEW_N, 0);
	...
	define_native_func_a(n, string_tobytes, 1, 2, NULL, NULL, fs->cls_bool);
	n = define_identifier(m, cls, "representable", NODE_FUNC_N, 0);
	define_native_func_a(n, sequence_is_valid_encoding, 1, 1, (void*) TRUE, NULL);
	extends_method(cls, fs->cls_sequence);
	...
	n = define_identifier_p(m, cls, fs->symbol_stock[T_LB], NODE_FUNC_N, 0);
	define_native_func_a(n, matches_index, 1, 1, NULL, fs->cls_int);
