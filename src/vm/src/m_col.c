 ...
 * [1,2,3] + [4,5]
 */
static int array_add(Value *vret, Value *v, RefNode *node)
{
	int i;
	RefArray *r1 = Value_vp(v[0]);
	RefArray *r2 = Value_vp(v[1]);
	RefArray *r = refarray_new(r1->size + r2->size);
	Value *rp = r->p;

	*vret = vp_Value(r);
	memcpy(rp, r1->p, r1->size * sizeof(Value));
	memcpy(rp + r1->size, r2->p, r2->size * sizeof(Value));
...
static int value_cmp_invoke(Value v1, Value v2, Value fn)
{
	Value v;
...
void define_lang_col_class(RefNode *m)
{
	...
	define_native_func_a(n, array_add, 1, 1, NULL, fs->cls_list);
