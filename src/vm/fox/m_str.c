...
static int matches_index(Value *vret, Value *v, RefNode *node)
{
    ...
    if (fg->stk_top > v + 1) {
        ...
        if (v1_type == fs->cls_int) {
            int64_t i64 = Value_int64(v[1], NULL);
            if (i64 < 0 || i64 >= r->count) {
            ...
            throw_error_select(THROW_ARGMENT_TYPE2__NODE_NODE_NODE_INT, fs->cls_int, fs->cls_str, v1_type, 1);
            return FALSE;
        }
    }
...
void define_lang_str_func(RefNode *m)
{
    ...
    define_native_func_a(n, str_hexdecode, 1, 3, NULL, NULL, fs->cls_charset, fs->cls_bool);

    n = define_identifier(m, m, "b64encode", NODE_FUNC_N, 0);
    define_native_func_a(n, str_base64encode, 1, 3, (void*) FALSE, NULL, fs->cls_charset, fs->cls_bool);

    n = define_identifier(m, m, "b64encode_url", NODE_FUNC_N, 0);
    define_native_func_a(n, str_base64encode, 1, 3, (void*) TRUE, NULL, fs->cls_charset, fs->cls_bool);

    n = define_identifier(m, m, "b64decode", NODE_FUNC_N, 0);
    define_native_func_a(n, str_base64decode, 1, 3, NULL, fs->cls_sequence, fs->cls_charset, fs->cls_bool);

    n = define_identifier(m, m, "urlencode", NODE_FUNC_N, 0);
    define_native_func_a(n, str_urlencode, 1, 3, (void*) FALSE, NULL, fs->cls_charset, fs->cls_bool);

    n = define_identifier(m, m, "urlencode_form", NODE_FUNC_N, 0);
    define_native_func_a(n, str_urlencode, 1, 3, (void*) TRUE, NULL, fs->cls_charset, fs->cls_bool);

    n = define_identifier(m, m, "urldecode", NODE_FUNC_N, 0);
    define_native_func_a(n, str_urldecode, 1, 3, NULL, fs->cls_sequence, fs->cls_charset, fs->cls_bool);
}
