...
{
    int err = FALSE;
    int flags = fs->Value_int64(v[1], &err);
...
static int sdlwindow_new(Value *vret, Value *v, RefNode *node)
{
    ...
    int x = fs->Value_int64(v[2], &err[0]);
    int y = fs->Value_int64(v[3], &err[1]);
    int w = fs->Value_int64(v[4], &err[2]);
    int h = fs->Value_int64(v[5], &err[3]);
    int flags = 0;

    if (fg->stk_top > v + 6) {
        flags = fs->Value_int64(v[6], &err[4]);
    }
