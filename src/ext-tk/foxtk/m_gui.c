...
void widget_event_handler_sub(Value *vret, Ref *r, RefStr *name)
{
	...
	if (hash == NULL) {
		...
		r->v[INDEX_WBASE_HANDLER] = handle_Value(hash);
	}
...
static int form_fixed(Value *vret, Value *v, RefNode *node)
{
	...
	size[0] = fs->Value_int(v[1], NULL);
	size[1] = fs->Value_int(v[2], NULL);
...
{
	WndHandle window = Value_widget_handle(*v);
	int width = fs->Value_int(v[1], NULL);
	int height = fs->Value_int(v[2], NULL);
...
{
	WndHandle window = Value_widget_handle(*v);
	int x = fs->Value_int(v[1], NULL);
	int y = fs->Value_int(v[2], NULL);
