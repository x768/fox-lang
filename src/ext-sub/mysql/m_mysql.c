...
static int client_check_open_and_throw_error(RefMySQLClient *my, RefNode *node)
{
    if (!my->valid) {
...
static int Value_to_MYSQL_BIND(MYSQL_BIND *mybind, Value v, Mem *mem)
{
...
static Value MYSQL_BIND_to_Value(MYSQL_BIND *b, RefMySQLCursor *cur, int n)
{
    if (*b->is_null) {
        return VALUE_NULL;
    }
...
{
    RefMySQLClient *my = fs->buf_new(cls_client, sizeof(RefMySQLClient));
    ...
    *vret = vp_Value(my);
    ...
    if (fg->stk_top > v + 5) {
        int err;
        int64_t port = fs->Value_int64(v[5], &err);
...
static int client_select_db(Value *vret, Value *v, RefNode *node)
{
    RefMySQLClient *my = Value_vp(*v);
    char *db = Value_cstr(v[1]);

    if (!client_check_open_and_throw_error(my, node)) {
        return FALSE;
    }
...
static int client_set_timezone(Value *vret, Value *v, RefNode *node)
{
    RefMySQLClient *my = Value_vp(*v);
    my->tz = Value_vp(v[1]);
    return TRUE;
...
static MYSQL_BIND *create_bind_param(Value *argv, int argc, int n_param, Mem *mem)
{
    ...
    for (i = 0; i < argc; i++) {
        if (!Value_to_MYSQL_BIND(&mybind[i], argv[i], mem)) {
            return NULL;
        }
    }
    return mybind;
...
static int client_query(Value *vret, Value *v, RefNode *node)
{
        ...
        *vret = int32_Value(mysql_stmt_affected_rows(stmt));
        ...
        if (cur.result != NULL) {
            ...
            case MYSQL_DATA_TRUNCATED:
                *vret = MYSQL_BIND_to_Value(&cur.result[0], &cur, 0);
                break;
            default:
                break;
            }
    ...
    if (cur->is_map) {
        ...
        *vret = vp_Value(rm);
        ...
        for (i = 0; i < num; i++) {
            ...
            ve->val = MYSQL_BIND_to_Value(&cur->result[i], cur, i);
        }
    } else {
        int i;
        RefArray *r = fs->refarray_new(num);
        *vret = vp_Value(r);
        ...
        for (i = 0; i < num; i++) {
            r->p[i] = MYSQL_BIND_to_Value(&cur->result[i], cur, i);
        }
    }
...
static void define_class(RefNode *m)
{
    ...
    cls_client = fs->define_identifier(m, m, "MySQLClient", NODE_CLASS, 0);
    cls_cursor = fs->define_identifier(m, m, "MySQLCursor", NODE_CLASS, 0);


    cls = cls_client;
    n = fs->define_identifier_p(m, cls, fs->str_new, NODE_NEW_N, 0);
    fs->define_native_func_a(n, client_new, 4, 5, NULL, fs->cls_str, fs->cls_str, fs->cls_str, fs->cls_str, fs->cls_int);
    ...
    fs->define_native_func_a(n, client_select_db, 1, 1, NULL, fs->cls_str);
    n = fs->define_identifier(m, cls, "timezone=", NODE_FUNC_N, 0);
    fs->define_native_func_a(n, client_set_timezone, 1, 1, NULL, fs->cls_timezone);
    n = fs->define_identifier(m, cls, "exec", NODE_FUNC_N, 0);
    fs->define_native_func_a(n, client_query, 1, -1, (void*) QUERY_EXEC, fs->cls_str);
    n = fs->define_identifier(m, cls, "query", NODE_FUNC_N, 0);
    fs->define_native_func_a(n, client_query, 1, -1, (void*) QUERY_ARRAY, fs->cls_str);
    n = fs->define_identifier(m, cls, "query_map", NODE_FUNC_N, 0);
    fs->define_native_func_a(n, client_query, 1, -1, (void*) QUERY_MAP, fs->cls_str);
    n = fs->define_identifier(m, cls, "query_one", NODE_FUNC_N, 0);
    fs->define_native_func_a(n, client_query, 1, -1, (void*) QUERY_SINGLE, fs->cls_str);
